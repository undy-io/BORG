...
func TestRunUsesChartConfigMapAndEffectiveSecretName(t *testing.T) {
	key := []byte("ffffffffffffffffffffffffffffffff")
	client := fake.NewSimpleClientset(
		&corev1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{Name: "prod-borg-config", Namespace: "models"},
			Data: map[string]string{
				"auth-secret-name": "central-borg-auth",
				"config.yaml": `
borg:
  auth_key_from_env: CUSTOM_AUTH_KEY
  auth_prefix: "CUSTOM:"
`,
			},
		},
		&corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{Name: "central-borg-auth", Namespace: "models"},
			Data: map[string][]byte{
				"CUSTOM_AUTH_KEY": encodedSecretKey(key),
			},
		},
	)
	...
	assertTokenUsername(t, strings.TrimSpace(stdout), key, "CUSTOM:", "alice")
	if stderr != "" {
		t.Fatalf("expected empty stderr, got %q", stderr)
	}
}
...
`,
		...
		&corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{Name: "borg-auth", Namespace: "models"},
			Data: map[string][]byte{
				"OVERRIDE_KEY": encodedSecretKey(key),
			},
		},
	)
...
func TestRunAcceptsUnpaddedSecretKeyText(t *testing.T) {
	key := []byte("uuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu")
	client := fake.NewSimpleClientset(
		&corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{Name: "borg-auth", Namespace: "models"},
			Data: map[string][]byte{
				"BORG_AUTH_KEY": []byte(base64.RawURLEncoding.EncodeToString(key)),
			},
		},
	)
	...
	assertTokenUsername(t, strings.TrimSpace(stdout), key, config.DefaultAuthPrefix, "uma")
}
...
func TestRunIgnoresMissingConfigMapOnly(t *testing.T) {
	...
	client := fake.NewSimpleClientset(
		&corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{Name: "borg-auth", Namespace: "models"},
			Data: map[string][]byte{
				"BORG_AUTH_KEY": encodedSecretKey(key),
			},
		},
	)
...
func TestRunRejectsRawSecretKey(t *testing.T) {
	key := []byte("eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee")
	client := fake.NewSimpleClientset(
		&corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{Name: "borg-auth", Namespace: "models"},
	...
	if !strings.Contains(err.Error(), "printable URL-safe base64 auth key") {
		t.Fatalf("unexpected error: %v", err)
	}
}

func TestRunRejectsInvalidPrintableSecretKeys(t *testing.T) {
	for name, value := range map[string]string{
		"empty auth mode": "EMPTY",
		"malformed text":  "not-a-valid-auth-key",
	} {
		t.Run(name, func(t *testing.T) {
			...
			if err == nil {
				t.Fatal("expected invalid printable Secret key to be rejected")
			}
			if !strings.Contains(err.Error(), "printable URL-safe base64 auth key") {
				t.Fatalf("unexpected error: %v", err)
			}
...
func TestParseOptionsAcceptsInterspersedFlags(t *testing.T) {
	...
	opts, err := parseOptions([]string{"alice", "-n", "models", "-r", "borg", "--secret-name", "central-auth"}, &stderr)
	if err != nil {
		t.Fatal(err)
	}
	if opts.secretName != "central-auth" {
		t.Fatalf("expected explicit secret name, got %q", opts.secretName)
	}
}
...
func encodedSecretKey(key []byte) []byte {
