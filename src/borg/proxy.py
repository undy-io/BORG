...
class RoundRobinSet:
    def __init__(self) -> None:
        ...
        self._cycler: Iterator[tuple[str, dict[str, Any]]] | None = None
...
class ProxyService:
    ...
    def __init__(
        ...
        """
        self._instances: dict[str, RoundRobinSet] = {}
    ...
    def require_auth(self, request: Request) -> str:  # noqa: D401
        ...
        if self.auth_key is None:
            raise HTTPException(
                status.HTTP_500_INTERNAL_SERVER_ERROR, "Auth key not configured"
            )
        ...
        if auth_prefix is None:
            raise HTTPException(
                ...
                "Auth prefix not configured",
            )

        plaintext = self._decrypt_token(auth[len(self.api_prefix) :], self.auth_key)

        if not plaintext.startswith(auth_prefix):
            raise HTTPException(status.HTTP_401_UNAUTHORIZED, "Invalid API key")

        username = plaintext[len(auth_prefix) :]
    ...
    async def remove_instance(
        ...
        async with self._lock:
            target_models = (
                list(self._instances.keys()) if models is None else set(models)
            )

            for model in target_models:
                self._rmv_ep(model, endpoint)

    async def pick_endpoint(self, model: str) -> tuple[str, dict[str, Any]]:
        """
        ...
        async with self._lock:
            return {
                ...
                "data": [
                    ...
                    for model, rrset in sorted(self._instances.items())
                    if rrset
                ],
            }

    async def proxy_request(
        self, model: str, request: Request, raw_body: bytes
    ) -> Response:
        """
    ...
    async def proxy(
        ...
        * Delegates to :pymeth:`ProxyService.proxy_request` or
        ...
        if not model:
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, "Missing 'model' in request body"
            )
        ...
        try:
            ...
            logger.exception("Fault occurred proxying the request")
            raise
