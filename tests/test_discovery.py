...
from borg.k8s_discovery import Endpoint, K8SDiscoveryService
...
class TestEndpoint:
    ...
    def test_endpoint_creation(self):
        ...
        endpoint = Endpoint(
            endpoint="http://localhost:8000/v1/models", models=models, apikey="EMPTY"
        )
...
class TestK8SDiscoveryService:
    ...
    @patch("borg.k8s_discovery.client.CoreV1Api")
    def test_init_kube_config_fallback(
        self, mock_core_v1, mock_load_kube, mock_load_incluster
    ):
    ...
    async def test_enum_models_success(self):
        ...
        with patch("borg.k8s_discovery.aiohttp.ClientSession") as mock_session_class:
            ...
            result = await K8SDiscoveryService._enum_models(
                "http://test:8000/v1/models"
            )
    ...
    async def test_discover_with_automodel(self, mock_service, mock_pod):
        ...
        mock_pod.metadata.annotations = {"protocol": "http", "apibase": "/v1"}
        ...
        with patch.object(
            K8SDiscoveryService,
            "_enum_models",
            return_value=["auto-model1", "auto-model2"],
        ):
            endpoints = []
            async for endpoint in mock_service._discover(
                namespace="default", selector="app=vllm", modelkey=None, automodel=True
            ):
                endpoints.append(endpoint)
    ...
    async def test_discover_no_models(self, mock_service, mock_pod):
        ...
        endpoints = []
        async for endpoint in mock_service._discover(
            namespace="default", selector="app=vllm", modelkey=None, automodel=False
        ):
            endpoints.append(endpoint)
...
class TestIntegration:
    ...
    async def test_full_discovery_cycle(self):
        ...
        with (
            ...
            mock_pods_response = Mock()
            mock_pods_response.items = [mock_pod]
            mock_core_v1.return_value.list_namespaced_pod.return_value = (
                mock_pods_response
            )
